...
#define Ws 5//Dimensione della finestra di invio
...
{
        sc_core::sc_time timeOut (420600, sc_core::SC_US);
        ...
        while (true)
        {
                std::cout << "\n####check " << sc_core::sc_time_stamp() << ", " << StatusMap[base].sendingTime << std::endl;
                //controllo timeout
                ...
                if(nextSN < base + Ws)
                {
                        /*if(nextSN <= nextSNTemp - 1)
                        {
                                ...
                                //Associo, utlizzando una map, il seqNum alla struct
                                //StatusMap[nextSN] = PacketStatus;
